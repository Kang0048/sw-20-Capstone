...
function generateImagePrompt(weatherData) {
    ...
    // 날씨 상태에 따라 프롬프트 생성
    const prompt = `A ${sky.toLowerCase()} day in ${location}, with an average temperature of ${avgTemp}°C, 
    ranging from a low of ${minTemp}°C to a high of ${maxTemp}°C.`;
...
// 실행 함수
async function main() {
    const location = 'seoul'; // 기본 지역 설정. 다른 지역 사용 시 이름 변경 ('gwangju', 'daegu' 등)
