    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
              {
                role: 'system',
                content: `You are a fashion assistant that generates detailed image prompts based on the season, weather, item, and prompt components provided by the user. 
                          You also have to convert userInput to engilsh if it's language is korean.
                          The image should depict a male model wearing the specified outfit, with a background that reflects the season and weather.
                          Keep the prompt under 1000 characters. Ensure there is absolutely no text in the image. Ensure that only the lower part of a person's face is visible in the image."`
              },
              {
                role: 'user',
                content: `User input: ${userInput}. Season: ${userSeason}. Weather: ${userWeather}. User item: ${userItem}`
              }
            ],
          });
        ...
        */
        ...
        console.log(`https://www.musinsa.com/search/goods?keyword=${userItem}&keywordType=keyword&gf=A`);
